...
resource "aws_eks_cluster" "arik-service1" {
  name     = "arik-service1"
...
resource "aws_launch_template" "eks_worker" {
  name                   = "worker"
  ebs_optimized          = true
  image_id               = "ami-06f7723e1b5a7ac2d"
  ...
  user_data = base64encode(templatefile(
    "templates/user-data.tpl",
    {
      ...
      cluster_auth_base64 = aws_eks_cluster.arik-service1.certificate_authority[0].data,
      cluster_endpoint    = aws_eks_cluster.arik-service1.endpoint,
      cluster_name        = "arik-service1",
    }
  ))
...
resource "aws_autoscaling_group" "eks_worker" {
  ...
  tag {
    key                 = "kubernetes.io/cluster/arik-service1"
    value               = "owned"
    propagate_at_launch = true
  }

  tag {
    key                 = "k8s.io/cluster/arik-service1"
    value               = "owned"
    propagate_at_launch = true
  }
